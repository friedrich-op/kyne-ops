...
const RIDERS = {
  ...
  IDIMU: [
    ...
    "Mr Sunday", "Mr Kenny", "Olawunmi", "Mr Wilson", "Mr Idris", "Azeez",
  ],
...
const VENDORS = {
  ...
  "EDZU": ["Davigormax","Davigormax powder","Venature","Yilest","QISE Anti wrinkle","A pack","Shilajit capsule","Retinol lotion"],
  ...
  "NUTRICARE": ["Beyond the like","Not just a girl","Stroke fighter","Fresh breath","Good breath","Asthma fighter","Ultimate stroke fighter","Diabetes ultra"],
  "MIRAC GLOBAL": ["Liver restore","Sperm revive capsule","Infection flusher","Hepafix","Infection crusher","Hepaflush"],
  "ADEITAN NETFAIR": ["Cucumber oil","Liver restore","Liver cur","Asthmacur","Lung cleanser","Infection flusher","Infection crusher","Ivision capsule","Eye cleanser tea","Detox tea","Ovucare tea","Fertility plus","Sleep cur","Knacker boost","Knacker capsule","Firm fix","Cemenplus","Sperm revive","Alpha man capsule","Alpha man syrup","Knacker plus","Fibguard","Fibroid flush","Prostaflush","Immune booster","Fibroid cur","Fertile cur","Prostaclen","Pile fix","Liver purifier","Liver forte","Fibroid cure","Florawet"],
            ...
            {unassigned.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {unassigned.map(o => {
                  ...
                  return (
                    <div key={o.id} style={{ background: "#fff", border: "1.5px solid #fecaca", borderRadius: "var(--r)", overflow: "hidden", boxShadow: "var(--shadow)" }}>
                      {/* Header — name + total */}
                      <div style={{ background: "#fef2f2", padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <p style={{ fontSize: "15px", fontWeight: 700, color: "var(--text)" }}>{o.customerName}</p>
                          {o.phone && <p style={{ fontSize: "13px", color: "var(--blue)", marginTop: "2px", fontWeight: 600 }}>📞 {o.phone}</p>}
                        </div>
                        <span style={{ fontFamily: "var(--display)", fontSize: "16px", fontWeight: 800, color: "var(--blue)" }}>{fmt(total)}</span>
                      </div>
                      {/* Details */}
                      <div style={{ padding: "12px 14px" }}>
                        {/* Address */}
                        <p style={{ fontSize: "12px", color: "var(--text-faint)", marginBottom: "10px" }}>📍 {o.address} · {o.date}</p>
                        {/* Products */}
                        <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "12px" }}>
                          {prods.map((p, i) => (
                            <div key={i} style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)", padding: "8px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              <div>
                                <p style={{ fontSize: "13px", fontWeight: 600, color: "var(--text)" }}>{p.name}</p>
                                <p style={{ fontSize: "11px", color: "var(--text-faint)", marginTop: "2px" }}>Qty: {p.qty}</p>
                              </div>
                              <p style={{ fontFamily: "var(--display)", fontSize: "14px", fontWeight: 700, color: "var(--blue)" }}>{fmt(p.price)}</p>
                            </div>
                          ))}
                        </div>
                        {/* Assign rider */}
                        <div style={{ borderTop: "1px solid var(--border)", paddingTop: "12px" }}>
                          <label className="k-label">Assign to Rider</label>
                          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                            <select
                              value={assignRider[o.id] || ""}
                              onChange={e => setAssignRider(p => ({ ...p, [o.id]: e.target.value }))}
                              className="k-input"
                              style={{ flex: 1, fontWeight: 600, fontSize: "15px" }}>
                            ...
                            <button
                              ...
                              style={{
                                padding: "12px 20px",
                                background: !assignRider[o.id] ? "#f1f5f9" : "var(--green)",
                                border: "none", borderRadius: "var(--r-sm)",
