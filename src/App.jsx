...
const GS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 640px) {
    /* Inputs — prevent iOS zoom, bigger touch targets */
    .k-input { font-size: 16px !important; padding: 12px 14px !important; }
    .k-label { font-size: 13px !important; margin-bottom: 7px !important; }
    button { min-height: 44px; }
    /* 2-col grids → 1-col on small screens */
    .grid-mob-1 { grid-template-columns: 1fr !important; }
    .grid-mob-2 { grid-template-columns: 1fr 1fr !important; }
    /* Reduce page padding */
...
function RiderManagerView({ branch, onLogout }) {
  ...
  const blankOrder   = { date: TODAY, customerName: "", phone: "", address: "", products: [{ ...blankProduct }] };
  ...
  const [assignCustomDateEnd, setAssignCustomDateEnd] = useState("");
  ...
  useEffect(() => {
    const t = setTimeout(() => setPhoneSearchD(phoneSearch), 300);
  ...
  const filteredOrders = useMemo(() => filterByPeriod(orders, mode, customDate, customDateEnd), [orders, mode, customDate, customDateEnd]);
  const pending    = useMemo(() => filteredOrders.filter(o => o.status === "Pending"),   [filteredOrders]);
  const delivered  = useMemo(() => filteredOrders.filter(o => o.status === "Delivered"), [filteredOrders]);
  const failed     = useMemo(() => filteredOrders.filter(o => o.status === "Failed"),    [filteredOrders]);
  const searchList = useMemo(() => {
    ...
    return unassigned.filter(o => {
      ...
      return phone.includes(q) || (o.customerName||"").toLowerCase().includes(q) || (o.address||"").toLowerCase().includes(q);
    });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: "720px", margin: "0 auto", padding: "16px 12px" }}>
            ...
            <Card>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
                ...
                <div>
                  ...
                  <input className="k-input" placeholder="e.g. 08012345678" type="tel" inputMode="numeric" value={form.phone||""} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
                </div>
                <div>
                ...
                <div key={i} style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)", padding: "10px", marginBottom: "10px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "8px" }}>
                    <div>
                      ...
                      <VendorCombo
                        value={p.vendor || ""}
                        onChange={v => setProduct(i, "vendor", v)}
                      />
                    </div>
                    <div>
              ...
              {(() => {
                ...
                return (
                  <button onClick={handleSubmit} disabled={disabled} style={{
                    ...
                    background: disabled ? "#f1f5f9" : "linear-gradient(135deg,var(--blue),var(--blue-dk))",
                    border: "none", borderRadius: "var(--r-sm)",
                    color: disabled ? "#94a3b8" : "#fff",
                    fontFamily: "var(--display)", fontSize: "15px", fontWeight: 800,
                    cursor: disabled ? "not-allowed" : "pointer",
                    boxShadow: disabled ? "none" : "0 4px 14px rgba(26,86,219,.3)",
                    letterSpacing: ".02em",
                  }}>Save Order →</button>
                );
              })()}
        ...
        {tab === "assign" && (
          <div className="fade-in">
            ...
            <div style={{ marginBottom: "12px" }}>
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: assignMode === "range" ? "8px" : "0" }}>
                {[["today","Today"],["yesterday","Yesterday"],["month","Month"],["range","Range"],["all","All"]].map(([id, lbl]) => (
                  <button key={id} onClick={() => { setAssignMode(id); setPhoneSearch(""); setExpandedOrderId(null); setAssignShowMore(50); }} style={{
                    padding: "6px 14px", borderRadius: "99px", fontSize: "12px", fontWeight: 600, cursor: "pointer", minHeight: "36px",
              ...
              {assignMode === "range" && (
                <div style={{ display: "flex", alignItems: "center", gap: "8px", background: "var(--blue-pale)", border: "1.5px solid var(--blue-pale2)", borderRadius: "var(--r-sm)", padding: "8px 12px", marginTop: "8px" }}>
                  <span style={{ fontSize: "11px", color: "var(--text-dim)", fontWeight: 600, whiteSpace: "nowrap" }}>From</span>
                  <input type="date" value={assignCustomDate} onChange={e => setAssignCustomDate(e.target.value)} className="k-input" style={{ flex: 1, fontSize: "13px", padding: "6px 8px" }}/>
                  <span style={{ fontSize: "11px", color: "var(--text-dim)", fontWeight: 600, whiteSpace: "nowrap" }}>To</span>
                  <input type="date" value={assignCustomDateEnd} onChange={e => setAssignCustomDateEnd(e.target.value)} className="k-input" style={{ flex: 1, fontSize: "13px", padding: "6px 8px" }}/>
                </div>
              )}
            </div>
            {unassigned.length === 0 ? (
              ...
              <>
                <div style={{ background: "var(--navy)", borderRadius: "var(--r-sm)", padding: "10px 14px", marginBottom: "12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <Tag label={assignMode === "today" ? "Today" : assignMode === "yesterday" ? "Yesterday" : assignMode === "month" ? "Month" : assignMode === "range" ? "Range" : "All"} type="navy" />
                </div>
                <input className="k-input" placeholder="Search by name, phone or address..." value={phoneSearch}
                ...
                {(() => {
                  ...
                  if (searchList.length === 0) return <p style={{ textAlign: "center", padding: "32px 0", fontSize: "13px", color: "var(--text-faint)" }}>No matches found</p>;
                  return (
                    <>
                      {phoneSearchD.trim() && <p style={{ fontSize: "11px", color: "var(--text-faint)", marginBottom: "8px" }}>{searchList.length} match{searchList.length !== 1 ? "es" : ""}</p>}
                        ...
                        {visible.map(o => {
                          ...
                          return (
                            <div key={o.id} style={{ background: "#fff", border: "1.5px solid " + (isExp ? "var(--blue)" : "var(--border)"), borderRadius: "var(--r-sm)", overflow: "hidden" }}>
                              <div onClick={() => setExpandedOrderId(isExp ? null : o.id)} style={{ padding: "10px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", gap: "8px" }}>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <p style={{ fontSize: "11px", color: "var(--text-faint)", marginTop: "1px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{o.phone ? o.phone + " · " : ""}{o.address}</p>
                                </div>
                              ...
                              {isExp && (
                                <div style={{ borderTop: "1px solid var(--border)", padding: "10px 12px", background: "var(--surface2)" }}>
                                  ...
                                  <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                                    <select value={selR} onChange={e => setAssignRider(p => ({ ...p, [o.id]: e.target.value }))} className="k-input" style={{ flex: 1, fontWeight: 600 }} onClick={e => e.stopPropagation()}>
                                      <option value="">Select rider...</option>
                                      {RIDERS[branch].map(r => <option key={r} value={r}>{r}</option>)}
                                    </select>
                                    <button onClick={e => { e.stopPropagation(); assignOrder(o.id); setExpandedOrderId(null); }} disabled={!selR}
                                      style={{ padding: "10px 16px", flexShrink: 0, background: !selR ? "#f1f5f9" : "var(--green)", border: "none", borderRadius: "var(--r-sm)", color: !selR ? "#94a3b8" : "#fff", fontFamily: "var(--display)", fontSize: "13px", fontWeight: 700, cursor: !selR ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}>
                                      Assign
                                    </button>
                                  </div>
                                </div>
        ...
        {tab === "update" && (
          <div className="fade-in">
            <SectionTitle title="Update Orders" sub="Select a rider to update their orders" />
            ...
            {/* Guard for wide periods */}
            {!selectedRider && (mode==="week"||mode==="month"||mode==="range"||mode==="all") ? (
              ...
              const vP = (selectedRider ? pending.filter(o=>o.rider===selectedRider) : pending).slice(0,50);
              const vD = (selectedRider ? delivered.filter(o=>o.rider===selectedRider) : delivered).slice(0,50);
              const vF = (selectedRider ? failed.filter(o=>o.rider===selectedRider) : failed).slice(0,50);
              const allVis = [...vP,...vD,...vF];
              const dSeen={}; const dupIds=new Set();
              allVis.forEach(o=>{ const k=(o.customerName||"").trim().toLowerCase()+"|"+(o.phone||"").replace(/^'/,"").trim()+"|"+o.date; if(dSeen[k]){dupIds.add(o.id);dupIds.add(dSeen[k]);}else dSeen[k]=o.id; });
              const pGroups={}; vP.forEach(o=>{const r=o.rider||"Unassigned";if(!pGroups[r])pGroups[r]=[];pGroups[r].push(o);});
              const dGroups={}; vD.forEach(o=>{const r=o.rider||"Unassigned";if(!dGroups[r])dGroups[r]=[];dGroups[r].push(o);});
              return (
                <>
                  {dupIds.size>0 && <div style={{background:"#fffbeb",border:"1.5px solid #fde68a",borderRadius:"var(--r-sm)",padding:"10px 14px",marginBottom:"12px",display:"flex",alignItems:"center",gap:"10px"}}><span style={{fontSize:"18px"}}>⚠️</span><div><p style={{fontSize:"13px",fontWeight:700,color:"var(--amber)"}}>{dupIds.size} possible duplicate order{dupIds.size!==1?"s":""}</p><p style={{fontSize:"11px",color:"#92400e",marginTop:"2px"}}>Same customer, phone and date</p></div></div>}
                  {vP.length>0&&<><p style={{fontSize:"11px",fontWeight:600,color:"var(--amber)",textTransform:"uppercase",letterSpacing:".06em",marginBottom:"12px"}}>Pending ({vP.length})</p>
                  {Object.entries(pGroups).map(([rider,ords])=>(
                    <div key={rider} style={{marginBottom:"20px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"8px",padding:"8px 12px",background:"var(--navy)",borderRadius:"var(--r-sm)"}}><RiderAvatar name={rider} size={26}/><span style={{fontFamily:"var(--display)",fontSize:"13px",fontWeight:700,color:"#fff"}}>{rider}</span><span style={{fontSize:"11px",color:"rgba(255,255,255,.45)",marginLeft:"auto"}}>{ords.length} order{ords.length!==1?"s":""}</span></div>
                        ...
                        {ords.map(o=>(
                          <div key={o.id} style={{background:dupIds.has(o.id)?"#fffbeb":"#fff",border:"1.5px solid "+(dupIds.has(o.id)?"#fde68a":"var(--border)"),borderRadius:"var(--r)",padding:"12px 14px",boxShadow:"var(--shadow)"}}>
                            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"8px"}}>
                              <div style={{flex:1}}>
                                <div style={{display:"flex",alignItems:"center",gap:"6px"}}><p style={{fontSize:"14px",fontWeight:700}}>{o.customerName}</p>{dupIds.has(o.id)&&<span style={{fontSize:"10px",fontWeight:700,color:"var(--amber)",background:"#fde68a",padding:"1px 6px",borderRadius:"99px"}}>DUP</span>}</div>
                  ...
                  {vD.length>0&&<><p style={{fontSize:"11px",fontWeight:600,color:"var(--green)",textTransform:"uppercase",letterSpacing:".06em",marginBottom:"12px"}}>Delivered ({vD.length})</p>
                  {Object.entries(dGroups).map(([rider,ords])=>(
                    <div key={rider} style={{marginBottom:"20px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"8px",padding:"8px 12px",background:"var(--green)",borderRadius:"var(--r-sm)"}}><RiderAvatar name={rider} size={26}/><span style={{fontFamily:"var(--display)",fontSize:"13px",fontWeight:700,color:"#fff"}}>{rider}</span><span style={{fontSize:"11px",color:"rgba(255,255,255,.55)",marginLeft:"auto"}}>{ords.length} delivered</span></div>
                      <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                        {ords.map(o=>(
                  ...
                  {vF.length>0&&<><p style={{fontSize:"11px",fontWeight:600,color:"var(--text-faint)",textTransform:"uppercase",letterSpacing:".06em",marginBottom:"8px"}}>Failed ({vF.length})</p>
                  <div style={{display:"flex",flexDirection:"column",gap:"8px",opacity:.6}}>{vF.map(o=>(<div key={o.id} style={{background:"#fff",border:"1.5px solid var(--border)",borderRadius:"var(--r)",padding:"10px 14px"}}><p style={{fontSize:"12px",fontWeight:500,color:"var(--text-dim)"}}>{o.customerName} · {o.rider}</p><p style={{fontSize:"11px",color:"var(--text-faint)",marginTop:"2px"}}>{getProducts(o).map(p=>p.name).join(", ")}</p></div>))}</div></>}
                  {vP.length===0&&vD.length===0&&vF.length===0&&<p style={{textAlign:"center",padding:"48px 0",fontSize:"13px",color:"var(--text-faint)"}}>No orders in this period</p>}
                </>
              );
            })()}
          </div>
